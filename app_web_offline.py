...
def initialize_system():
    """Inicializa el sistema automáticamente sin descargar modelos"""
    if state.initialized:
        return True
    
    try:
        logger.info("Iniciando sistema AMSAC AI (modo offline)...")
        
        # Cargar Gemini (no necesita descarga)
        try:
        ...
        if documentos_path.exists():
            ...
            if pdf_files:
                ...
                if todos_los_chunks:
                    # Crear embeddings simples (basados en TF-IDF como fallback)
                    logger.info(f"📊 Creando embeddings simples para {len(todos_los_chunks)} fragmentos...")
                    try:
                        ...
                        logger.info("✅ Usando SentenceTransformers")
                    except Exception as e:
                        logger.warning(f"⚠️ No se pudo cargar SentenceTransformers: {e}")
                        logger.info("🔄 Usando embeddings simples como fallback")
                        # Crear embeddings simples basados en longitud y palabras clave
                        embeddings = np.array([
                            np.array([
                                len(chunk['content']),
                                chunk['content'].lower().count('iso'),
                                chunk['content'].lower().count('seguridad'),
                                chunk['content'].lower().count('certificacion'),
                                chunk['content'].lower().count('proceso'),
                                chunk['content'].lower().count('sistema'),
                                hash(chunk['content'][:100]) % 1000
                            ], dtype=np.float32) 
...
def buscar_similares(pregunta, k=8):
    ...
    try:
        ...
        if state.model:
            ...
            query_embedding = np.array([[
                len(pregunta),
                pregunta.lower().count('iso'),
                pregunta.lower().count('seguridad'),
                pregunta.lower().count('certificacion'),
                pregunta.lower().count('proceso'),
                pregunta.lower().count('sistema'),
                hash(pregunta[:100]) % 1000
            ]], dtype=np.float32)
...
async def get_status():
    ...
    try:
        ...
        return {
            ...
            "mode": "offline" if state.model is None else "online"
        }
    except Exception as e:
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("app_web_offline:app", host="0.0.0.0", port=8000, reload=True)
